...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto p-6 md:p-10 min-h-screen pb-20">
      {/* Header */}
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
          <h1 className="text-2xl font-semibold text-text-primary mb-1">Leads Dashboard</h1>
          <p className="text-text-secondary text-[13px]">Manage your sales pipeline and conversion efficiency</p>
        </div>
      ...
      {user?.role === 'admin' && stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <StatCard icon={<Users size={18} />} label="Total Leads" value={stats.totalLeads} />
          <StatCard icon={<Check size={18} />} label="Qualified" value={stats.qualifiedLeads} />
          <StatCard icon={<TrendingUp size={18} />} label="New This Week" value={stats.newThisWeek} />
          <StatCard icon={<X size={18} />} label="Lost" value={stats.lostLeads} />
        </div>
      )}

      {/* Filter Bar */}
      <div className="bg-bg-surface border border-border rounded-xl p-[10px] mb-6 flex flex-wrap items-center gap-[10px]">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-bg-input border border-border rounded-lg outline-none focus:border-border-focus text-[13px] text-text-primary transition-colors placeholder:text-text-hint"
          />
        </div>
        ...
        <select
          ...
          className="bg-bg-input border border-border rounded-lg h-9 px-3 text-[13px] text-text-secondary outline-none focus:border-border-focus"
        >
        ...
        {hasFilters && (
          <button
            ...
            className="text-text-hint hover:text-text-primary text-[12px] flex items-center gap-1.5 px-2 transition-colors"
          >
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-bg-elevated border-b border-border">
                <th className="px-6 py-3 text-left col-header">Name</th>
                <th className="px-6 py-3 text-left col-header">Email Address</th>
                <th className="px-6 py-3 text-left col-header">Status</th>
                <th className="px-6 py-3 text-left col-header">Source</th>
                <th className="px-6 py-3 text-left col-header">Created At</th>
                <th className="px-6 py-3 text-right col-header">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                leads.map((lead, i) => (
                  <tr
                    key={lead._id}
                    className={`border-b border-border data-row hover:bg-accent-soft group ${i % 2 === 0 ? 'bg-bg-surface' : 'bg-bg-elevated/30'}`}
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-bg-elevated flex items-center justify-center text-[11px] font-bold text-text-secondary group-hover:bg-accent/20 group-hover:text-accent transition-colors">
                          {lead.name.charAt(0).toUpperCase()}
                        </div>
                        <Link to={`/leads/${lead._id}`} className="text-[13px] font-medium text-text-primary hover:text-accent transition-colors">
                          {lead.name}
                        </Link>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[13px] text-text-secondary">{lead.email}</td>
                    <td className="px-6 py-4">
                      <StatusBadge status={lead.status} />
                    </td>
                    <td className="px-6 py-4 text-[13px] text-text-secondary">{lead.source}</td>
                    <td className="px-6 py-4 text-[12px] text-text-hint">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          to={`/leads/${lead._id}`}
                          title="View Details"
                          className="w-7 h-7 flex items-center justify-center bg-accent/10 text-accent hover:bg-accent/20 rounded-lg transition-colors"
                        >
                          <Eye size={14} />
                        </Link>
                        <button
